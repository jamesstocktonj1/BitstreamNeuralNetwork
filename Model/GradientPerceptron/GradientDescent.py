...
#     # get initial correct values
#     y_hat = neuron.call(x)
#     correct = ((y_hat > 0.5) == y) * 1


#     #while (trainCount < TRAIN_COUNT) and (correct.sum() != X):
...
#         grads = neuron.diff(y_in, x_in)
#         #print("Grads ", grads)

#         weights -= r * (grads.sum() / grads.shape[0])
...
#         print("Correct Points: ", correct.sum())
...
#             grad = neuron.diff(y[rxy], np.array([x[rxy][0], x[rxy][1]]))
#             print("Grad ", grad)
#             print("Weights, ", weights)
...
def plot_decision(neuron, x, y):
    ...
    plt.xlabel('x')
    plt.ylabel('y')
    plt.savefig("images/perceptron_decision_gradient.png")
    plt.close()
